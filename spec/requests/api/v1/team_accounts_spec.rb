...
RSpec.describe "Api::V1::TeamAccounts", type: :request do
  let(:user) { User.create(email: 'test@example.com', password: 'password') }
  describe "Post /api/v1/team_accounts" do
    let(:jwt_token) { AuthenticationService.generate_jwt(user.id) }
    context 'with valid credentials' do
      before do
        post '/api/v1/team_accounts', params: {name: "team_accounts"}, headers: { 'Authorization' => "Bearer #{jwt_token}" }
      end
